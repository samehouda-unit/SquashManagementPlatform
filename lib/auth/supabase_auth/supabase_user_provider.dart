...
class EgyptSquashPlatformSupabaseUser extends BaseAuthUser {
  EgyptSquashPlatformSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> egyptSquashPlatformSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EgyptSquashPlatformSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
